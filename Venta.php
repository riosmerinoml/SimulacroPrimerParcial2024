...
class Venta{
    ...
    public function __construct($numVentaParam, $fechaVentaParam, $objClienteParam, $colObjMotoParam, $precioFinalparam) {
        
        $this->numVenta=$numVentaParam;
        $this->fechaVenta=$fechaVentaParam;
        $this->objCliente=$objClienteParam;
        $this->colObjMoto=$colObjMotoParam;
        $this->precioFinal=$precioFinalparam;
         }
    ...
    public function setNumVenta($numVentaParam)
    {
        $this->numVenta = $numVentaParam;
    }

    public function setFechaVenta($fechaVentaParam)
    {
        $this->fechaVenta = $fechaVentaParam;
    }

    public function setObjCliente($objClienteParam)
    {
        $this->objCliente = $objClienteParam;
    }

    public function setColObjMoto($colObjMotoParam)
    {
        $this->colObjMoto = $colObjMotoParam;
    }

    public function setPrecioFinal($precioFinalparam)
    {
        $this->precioFinal = $precioFinalparam;
    }

    public function cadenaMotos($muchasMotos) {
        $cadena = "\n";
        foreach($muchasMotos as $unaMoto) {
            $cadena .= $unaMoto;
        }
        return $cadena;
    }
    ...
    public function __toString() {
        $cadena = "\nNúmero de venta: ".$this->getNumVenta().
                    "\nFecha de venta : ".$this->getFechaVenta().
                    "\nReferencia al cliente : ".$this->getObjCliente().
                    "\nReferencia a una colección de motos: ".$this->cadenaMotos($this->getColObjMoto).
    ...
    public function incorporarMoto($objMoto) {
        //Revisa que el cliente no haya sido dado de baja. De haber sido dado de baja, la moto no es incorporada.
        $objClienteAux = $this->getObjCliente();
        $bajaCliente = $objClienteAux->getBaja();
        if ($bajaCliente) {
        } else {
            //Revisa que la moto este activa.
            if($objMoto->getActiva()) {
                //Luego de confirmar que la moto esta activa, invoca la función darPrecioVenta y acumula este valor
                //en el atributo $precioFinal de este objeto.
                $precioParcial = $this->getPrecioFinal()+($objMoto->darPrecioVenta());
                $this->setPrecioFinal($precioParcial);
                //Además agrega la moto a $colObjMoto de este objeto Venta.
                $colObjMotoAux = $this->getColObjMoto();
                array_push($colObjMotoAux, $objMoto);
                $this->setColObjMoto($colObjMotoAux);
            } else {
                //Si la moto no esta activa, no es incorporada a la venta.
            }
        }
    }
