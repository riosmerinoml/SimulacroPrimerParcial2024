...
class Empresa {
    ...
    public function __construct($denominacionParam, $direccionParam, $colObjClienteParam, $colObjMotoParam, $colObjVentaParam) {

        $this->denominacion=$denominacionParam;
        $this->direccion=$direccionParam;
        $this->colObjCliente=$colObjClienteParam;
        $this->colObjMoto=$colObjMotoParam;
        $this->colObjVenta=$colObjVentaParam;
    }
    ...
    /**
     * Get the value of colObjCliente
     */ 
    ...
    /**
     * Set the value of colObjCliente
     *
     * @return  self
     */ 
    ...
    public function mostrarElementosDeUnaColeccion($coleccion) {
        $cadena = " ";
    ...
    //Redefinición de la función toString.
    public function __toString() {
        $cadena = "\nDenominación: ".$this->getDenominacion()
            ."\nDirección: ".$this->getDireccion()
            ."\nClientes: ".$this->mostrarElementosDeUnaColeccion($this->getColObjCliente())
            ."\nMotos: ".$this->mostrarElementosDeUnaColeccion($this->getColObjMoto())
            ."\nVentas: ";$this->mostrarElementosDeUnaColeccion($this->getColObjVenta);
    }

    public function retornarObjetoMoto($codigoMoto) {
        $i = 0;
        $objMotoSalida = null;
        $totalObjMoto = count($this->getColObjMoto());
        $coleccionObjMoto = $this->getColObjMoto();
        while ($i < $totalObjMoto && (!$objMotoSalida || $objMotoSalida->getCodigoMoto() != $codigoMoto)) {
            if ($coleccionObjMoto[$i]->getCodigoMoto() == $codigoMoto) {
                $objMotoSalida = $coleccionObjMoto[$i];
            }
            $i++;
        }
        return $objMotoSalida;
    }
            ...
            foreach($colCodigosMoto as $codigo) {
                foreach($this->getColObjMoto() as $objMoto) {
                    if($objMoto->getActiva()){
                        if ($codigo == $objMoto->getCodigoMoto()){
                            $objVenta = new Venta();
                            $importaFinal += $objMoto->darPrecioVenta();
                            $objVenta->incorporarMoto($objMoto);
                            $objVenta = $this->getColObjVenta()[count(getColObjVenta())];
                            $this->setColObjVenta()[count(getColObjVenta())];
                        }
                    }
                }
            }
    ...
    public function retornarVentasXCliente($tipo, $numDoc) {
        foreach($this->getColObjVenta as $ObjVenta){
            ...
            if ($tipo == $objCliente->getTipoDocumento() && $numDoc == $objCliente->getNumDocumento()){
                $colObjVentasRealizadas = $objVenta;
            }
        }
        return $colObjVentasRealizadas;
    }
